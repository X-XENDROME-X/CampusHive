/**
 * <p> OTPController_user Class </p>
 *
 * <p> Description: This class handles the functionality for OTP generation, email sending, 
 * and password reset in the user interface. It includes methods for sending the OTP, 
 * validating it, and updating the user's password.</p>
 *
...
public class OTPController_user {
    ...
    private void changePassword() {
        ...
        String email = emailField.getText(); // Assuming you have an email variable in your class
        ...
        try {
            // Update the password in the H2 database
            boolean success = H2Database.updatePasswordByEmail(email, newPassword);
            if (success) {
                showError("Password changed successfully!");
            } else {
                showError("Failed to change password. Email not found.");
            }
    ...
    void handleHomeButtonAction(ActionEvent event) {
        try {
            ...
            currentStage.setScene(LoginPageScene);
            currentStage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
